...
O painel administrativo autenticado usa uma linguagem própria inspirada em lista operacional: navegação lateral laranja fixa no fluxo, marca grande no topo, itens de menu em blocos altos com ícones lucide e item ativo em superfície branca. Não use header superior no admin autenticado.

As páginas do admin devem abrir com ícone circular claro, título forte e contagem de registros. Filtros principais aparecem como abas horizontais sublinhadas em laranja; filtros auxiliares ficam à direita em controles compactos. Listas de planos e contas usam cabeçalho de colunas claro e linhas largas arredondadas, com fundo branco, borda sutil e sombra baixa apenas para separar linhas. Na lista de planos, as colunas seguem Tipo, Nome, Valor, Cobrança, Benefícios, Uso e Ação. Tipo mostra só ícone e rótulo, com ícone azul para planos públicos e laranja para personalizados. Cobrança mostra apenas o resumo principal. Benefícios usa três linhas obrigatórias, sem badges: emissão fiscal, limite de PDVs e limite de subcontas. Uso mostra usuários ativos nos planos públicos; em planos personalizados, mostra o e-mail do usuário vinculado ou o código único quando ainda não usado. Ação de linha fica no extremo direito com botão de ícone. Na lista de contas, não use coluna de ID. Priorize a coluna Conta para exibir o e-mail completo ou quebrado com naturalidade, seguido apenas do estado de verificação do e-mail em verde ou amarelo. O plano sem assinatura aparece em vermelho. O próximo pagamento fica como metadado pequeno abaixo do plano, não como coluna separada. A coluna Status representa a situação de pagamento com quatro estados: Sem assinatura, Em dia, Em atraso e Bloqueado; Bloqueado vale quando o atraso passa de 7 dias. As abas superiores da lista de contas começam com Todos e seguem com esses quatro estados. Use uma busca única e genérica para e-mail, plano, status, pagamento e uso; não adicione selects auxiliares nem botão Filtrar quando a busca local resolver. Evite sombras ou fundos cinza atrás das linhas da lista.

Modais do admin seguem um padrão compacto inspirado em formulário operacional: overlay escuro com blur de fundo, card branco centralizado, cantos arredondados, cabeçalho curto, botão de fechar discreto no canto superior direito, labels pequenas e campos com borda suave. Formulários do admin devem usar grid de duas colunas quando houver espaço, campos full-width apenas para informações longas, switches para opções booleanas e ações no rodapé à direita. Modais de planos permanecem centralizados quando couberem no viewport. O cadastro de planos deve manter o fluxo em duas etapas: primeiro escolha entre plano público e personalizado; depois abra o formulário do tipo escolhido. Nos formulários de planos, limite de subcontas e limite de PDVs são campos numéricos pareados na mesma linha. No formulário de plano público, nome e valor mensal ficam pareados na primeira linha, sem campo de observação. No formulário de plano personalizado, mantenha pares na mesma linha quando fizer sentido, como nome/observação, valor/dias grátis, cobrança por dias/dias entre cobranças e limite de subcontas/limite de PDVs. Para cobrança por dias, use switch simples, sem caixa auxiliar, e mostre o campo de quantidade somente quando o switch estiver ativo. O aviso de código gerado deve ser uma caixa pontilhada, alinhada à esquerda, com ícone à esquerda do texto. Evite cards internos por seção e excesso de texto.
