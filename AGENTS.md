...
- Headers de áreas operacionais, plataforma logada, dashboard e PDV devem ficar no topo do fluxo da página e rolar para fora da viewport quando o usuário descer a tela. Não use `position: fixed` ou `sticky` nesses headers sem pedido explícito. A única exceção é o header da landing page inicial do `web`, que pode ser menu flutuante.
